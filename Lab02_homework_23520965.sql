...
FROM KyNang 
WHERE LoaiKyNang NOT LIKE N'Ngôn ngữ lập trình'
...
SELECT DuAn.MaCongTy, CongTy.TenCongTy, Count(DuAn.MaDuAn) AS SoDuAn
FROM DuAn INNER JOIN CongTy ON DuAn.MaCongTy=CongTy.MaCongTy
WHERE (DuAn.TrangThai=N'Đang thực hiện') 
GROUP BY DuAn.MaCongTy, CongTy.TenCongTy
...
SELECT ChuyenGia_KyNang.MaKyNang, KyNang.TenKyNang, Count(ChuyenGia_KyNang.MaChuyenGia) AS SoChuyenGia
FROM ChuyenGia_KyNang INNER JOIN KyNang 
...
SELECT TenCongTy, AVG(SoNhanVien) AS SoNhanVienTB
FROM CongTy 
...
SELECT ChuyenGia_KyNang.MaChuyenGia, ChuyenGia.HoTen, COUNT(ChuyenGia_KyNang.CapDo) AS SoKyNang
FROM ChuyenGia_KyNang INNER JOIN ChuyenGia 
ON ChuyenGia_KyNang.MaChuyenGia=ChuyenGia.MaChuyenGia
WHERE ChuyenGia_KyNang.CapDo<=5
GROUP BY ChuyenGia_KyNang.MaChuyenGia, ChuyenGia.HoTen
